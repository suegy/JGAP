...
public class BranchTypingCross
  ...
  protected ProgramChromosome[] doCross(ProgramChromosome a_c0,
    ...
    if (random.nextFloat() < getConfiguration().getFunctionProb()) {
      ...
      if (random.nextDouble() <= getConfiguration().getMutationProb()) {
        if (IMutateable.class.isInstance(command)) {
          ...
          command = term.applyMutation(0, random.nextDouble());
          if (command != null) {
        	...
        	if (a_c1.getAvailableCommandOfClass(command.getClass()) >= 0 && 
        			  command.getArity(a_c1.getIndividual()) == a_c0.getGene(p0).getArity(a_c0.getIndividual())) {
      ...
      if (random.nextDouble() <= getConfiguration().getMutationProb()) {
        if (IMutateable.class.isInstance(command)) {
          ...
          if (command != null) {
            // Check if mutant's function is allowed.
            // --------------------------------------
        	  if (a_c0.getAvailableCommandOfClass(command.getClass()) >= 0 && 
        			  command.getArity(a_c0.getIndividual()) == a_c1.getGene(p1).getArity(a_c1.getIndividual())) {
    ...
    if (org.jgap.gp.function.SubProgram.class.isAssignableFrom(a_c1.getFunctions()[p1].getClass())) {
    	/* FIXME(swen) added a random mutation rate instead of arbitrary 0.5d */ 
      ((IMutateable)a_c1.getFunctions()[p1]).applyMutation(0, random.nextDouble());
    }
