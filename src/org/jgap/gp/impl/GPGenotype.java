...
implements Runnable, Serializable, Comparable {
	...
	private double m_prev_avg_prog_length;
	...
	private boolean parsimony;
	
	public boolean switchParsimonyPressure(boolean state){
		parsimony = state;
	 ...
	 * =
	 * Modified to reduce bloat of programs
	 *
	 * @author Klaus Meffert
	 * @since 3.0
	 * @author Swen E. Gaudl
	...
	public void calcFitness() {
		...
		for (int i = 0; i < pop.size() && pop.getGPProgram(i) != null; i++) {
			...
			fitnessOutPut += String.format("%s:%s;", fitness,program.getAdditionalFitnessInfo());
		}
		LOGGER.info("#########################");
		LOGGER.info("unscaled fitness: " + fitnessOutPut);
		fitnessOutPut = "";
		
		if (parsimony)
			applyCovriantParsimonyPressure(pop);
		...
		if (parsimony)
			LOGGER.info("parsimony-scaled fitness: " + fitnessOutPut);
		...
		/* FIXME(swen) changed it so that it includes only the fittest individual when this is really wished for*/
		if (m_allTimeBest != null && getGPConfiguration().isPreserveFittestIndividual()){
	...
	/**
	 * @author Swen Gaudl
	 * @param prog an IGPProgram containing functions and Terminals
	 * @return an integer value measuring the length opf the program
	 */
	...
	/**
	 * This method is used to apply a fitness pressure on the programs of the given population. 
	 * It follows the concepts described in Poli2008b by punishing bloat of shringage of programs.
	 * The current implementation does not use a modelling function which changes over the evolutionary timesteps, 
	 * but it changes the cavianrance punishment dynamicly better fitting the actual purpose.
	 * The method should be used before selection for the new generation.
	 * @author Swen Gaudl
	 * 
	 * @param pop a GPPopulation where parsimony pressure should be applied to the individual programs.
	 */
	private void applyCovriantParsimonyPressure(GPPopulation pop) {

		double sum_prog_length = 0;
		HashMap<Integer, Double> propability_of_prog_size = new HashMap<Integer,Double>();
		HashMap<Integer, Double> total_fitness_of_prog_size = new HashMap<Integer,Double>();
		double total_fitness = 0;
		double avg_fitness = 0;
		double cov_l_f = 0;
		double cov_l_l = 0;
		HashMap<Integer, Double> avg_fitness_of_prog_size = new HashMap<Integer,Double>();
		HashMap<Integer, Integer> nr_prog_size = new HashMap<Integer,Integer>();
		double avg_prog_length;

		for (IGPProgram igpProgram : pop.getGPPrograms()) {
		...
		avg_prog_length = sum_prog_length / pop.size();
		avg_fitness = total_fitness / pop.size();
		for (int l : total_fitness_of_prog_size.keySet()) {
			...
			cov_l_f += (l-avg_prog_length)*(avg_fitness_of_prog_size.get(l)-avg_fitness)*((double)(nr_prog_size.get(l))/pop.size());
			cov_l_l += (l-avg_prog_length)*(l-avg_prog_length)*((double)(nr_prog_size.get(l))/pop.size());
		}
		...
		for (IGPProgram igpProgram : pop.getGPPrograms()) {
			...
			double g_t = expactationCurve(pop.getGPConfiguration().getGenerationNr());
			
			//double c_t = cov_l_f/cov_l_l;
			double c_t = ( cov_l_f - (g_t - avg_prog_length) * avg_fitness ) / (cov_l_l - (g_t - avg_prog_length)*avg_prog_length);
	...
	/**
	 * used inside the parsimony pressure to match the expaction against
	 * @param generationNr
	 * @return
	 */
	private double expactationCurve(int generationNr){
		
		// a program should be at least 1 command lonG and the programs should slowly so the inclination is set to 
		
		return 1.0+m_prev_avg_prog_length;
	}
	...
	public void outputSolution(IGPProgram a_best) {
		...
		LOGGER.info("Generation: " + a_best.getGPConfiguration().getGenerationNr());
	...
	public void evolve() {
		try {
			...
			for (int i = 0; i < popSize1; i++) {
				...
				/**
				 * @TODO: there is an issue here and its that the ratio of crossover and reproduction is dependant on random distribution
				 * in the long run this leads to unstable chromosome pools so I am combining the ratio of crossover and reproduction into this
				 * setting the max number of both to a fixed value which guarantees this desired distribution
				 */

				// Note that if we only have one slot left to fill, we don't do
				// crossover, but fall through to reproduction.
				// ------------------------------------------------------------
				if (i < popSize1 - 1 && ((val < crossProb && crossover*2 < popSize1*crossProb ) || reproduction >= popSize1*(1-crossProb) ) ) {
					...
					do {
						try {
							...
							{
								/**
								 * FIXME(swen) introducing the counter reduce basically reverts to a new draw from the pool to allow for a new selection
								 * this can potentially introduce an infinite loop which is bad but its a quick fix without reverting to putting a completely new chromosome into the pool
								 */
								i--;
								checkblockage--;
								if (checkblockage <= 0)
