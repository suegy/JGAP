...
public abstract class BaseGPChromosome
  ...
  /**
   * Helper: Checks if the given class is availabe to the this chromosme based on the internal set of functions
   * The method returns the index of the Function in the FunctionSet. 
   * 
   * @author swen gaudl
   * @param a_class the class to find a command for
   * @return index of the found gene --- GP command, or -1 if none found
   */
  public int getAvailableCommandOfClass(Class a_class) {
		CommandGene[] functions = getFunctionSet();
	    int len = functions.length;
	    for (int j = 0; j < len && functions[j] != null; j++) {
	      if (functions[j].getClass() == a_class) {
	    	  return j;
